...
export class TestInitializerComponent implements OnInit {
  ...
  loggedUser: User = new User()//JSON.parse(localStorage[this.LS_CHAVE])
  ...
  initiateTimer(): void {
    ...
    TestInitializerComponent.testTime = 0
    this.nIntervId = setInterval(this.updateSeconds, 1000);
  }

  updateSeconds(): void {
    //console.log("update: " + TestInitializerV3Component.testTime)
    TestInitializerComponent.testTime = TestInitializerComponent.testTime + 1
  }

  get staticTestTime() {
    return TestInitializerComponent.testTime;
  }
