import { LeafEthernetCommunicationTag } from './../../../shared/models/tags/leaf-ethernet-communication-tag.model';
...
@Component({
  selector: 'app-leaf-ethernet-communication-tag',
  templateUrl: './leaf-ethernet-communication-tag.component.html',
  styleUrls: ['./leaf-ethernet-communication-tag.component.css']
})
export class LeafEthernetCommunicationTagComponent implements OnInit {
  @Input() tag!: LeafEthernetCommunicationTag
  @Input() tags!: BaseTag[]
  ...
  tagToUpdate!: LeafEthernetCommunicationTag
  newTag!: LeafEthernetCommunicationTag
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      this.disableDesiredFields()
    }, 1);
  }

  disableDesiredFields(): void {
    console.log(this.tag)
    if(this.tag != null) {
      if(this.tag.mainCommunication) {
        document.getElementById('registerToFlag')?.removeAttribute("disabled");
        document.getElementById('registerToSetTimeout')?.removeAttribute("disabled");
        document.getElementById('waitBetweenFeedback')?.removeAttribute("disabled");
      } else {
        //this.newTag.ensTagConfiguration = new EnsTagConfiguration()
        document.getElementById('registerToFlag')?.setAttribute("disabled", "disabled")
        document.getElementById('registerToSetTimeout')?.setAttribute("disabled", "disabled")
        document.getElementById('waitBetweenFeedback')?.setAttribute("disabled", "disabled")
      }
    } else {
  ...
  mainCommunicationChecker(): void {
    if(this.newTag.mainCommunication) {
      document.getElementById('registerToFlag')?.removeAttribute("disabled");
      document.getElementById('registerToSetTimeout')?.removeAttribute("disabled");
      document.getElementById('waitBetweenFeedback')?.removeAttribute("disabled");
    } else {
  ...
  mainCommunicationUpdateChecker(): void {
    if(this.tagToUpdate.mainCommunication) {
      document.getElementById('registerToFlag')?.removeAttribute("disabled");
      document.getElementById('registerToSetTimeout')?.removeAttribute("disabled");
      document.getElementById('waitBetweenFeedback')?.removeAttribute("disabled");
    } else {
