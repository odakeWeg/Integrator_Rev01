import { LeafVerifyMultipleTag } from './../../../shared/models/tags/leaf-verify-multiple-tag.model';
...
@Component({
  selector: 'app-leaf-verify-multiple-tag',
  templateUrl: './leaf-verify-multiple-tag.component.html',
  styleUrls: ['./leaf-verify-multiple-tag.component.css']
})
export class LeafVerifyMultipleTagComponent implements OnInit {
  @Input() tag!: LeafVerifyMultipleTag
  @Input() tags!: BaseTag[]
  ...
  tagToUpdate!: LeafVerifyMultipleTag
  newTag!: LeafVerifyMultipleTag
  ...
  update():void {
    ...
    try {
      this.tag.valueRef = this.formTag.value.valueRef!.split(",").map((el: string) => {return Number(el)})
    } catch (error) {
  ...
  add():void {
    ...
    try {
      this.newTag.valueRef = this.formNewTag.value.valueRef!.split(",").map((el: string) => {return Number(el)})
    } catch (error) {
      //@Todo: refactor this: try catch only used to check if the statement is a array or string
    }
    try {
